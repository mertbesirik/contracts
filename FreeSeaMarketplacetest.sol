...
import "https://github.com/mertbesirik/contracts/blob/main/FreeSeatest.sol";
import "https://github.com/mertbesirik/contracts/blob/main/PaymentGatewaytest.sol";

contract FreeSeaMarketplacetest is Ownable {

  FreeSeatest private _FreeSeatest;
  PaymentGatewaytest private _PaymentGatewaytest;
  ...
  constructor(address _FreeSeaContractAddress, address _PaymentGatewayAddress, address payable _publisherWallet) {
    _setFreeSeaContract(_FreeSeaContractAddress);
  ...
  function _setFreeSeaContract(address _FreeSeaContractAddress) private onlyOwner{
    _FreeSea = FreeSea(_FreeSeaContractAddress);
  }

  function setOffer(uint256 price, uint256 tokenId, address tokenAddress) public{
    require(_FreeSea.ownerOf(tokenId) == msg.sender, "Only the owner of the artwork is allowed to do this");
    require(_FreeSea.isApprovedForAll(msg.sender, address(this)) == true, "Not approved to sell");
    ...
    emit artworkAdded(address(_FreeSea), msg.sender, price, tokenId, offerId, false);
  }
  ...
  function buyArt(uint256 tokenId, address tokenAddress) public payable{
    ...
    _FreeSea.safeTransferFrom(offer.seller, msg.sender, tokenId);
  ...
  function _distributeFees(uint256 tokenId, uint256 price, address payable seller) internal{
    uint8 creatorRoyalty = _FreeSea.getRoyalty(tokenId);
    ...
    address payable creator = _FreeSea.getCreator(tokenId);
